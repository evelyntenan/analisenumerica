...
def main():
  ...
  i=0
  for i in range(n):    
    d.append(-12*h*h*h*(i+1)*(i+1)-2*h*h*h+12*h*h*(i+1)-2*h)
...
#resolver o sisteminha#
  ...
  i=1
  for i in range(n):
    ui.append((x0+i*h)*(x0+i*h)*(1-(x0+i*h))*(1-(x0+i*h)))
  i=1
  for i in range(n):
  ...
  for i in range(n+1):
    j=0
    for j in range(10):
      uj.append((x0+i*h+j*h/10)*(x0+i*h+j*h/10)*(1-(x0+i*h+j*h/10))*(1-(x0+i*h+j*h/10)))
  uj.append(0)
  #i=0
  #for i in range(10*(n+1)+1):
    #print(i,uj[i])
